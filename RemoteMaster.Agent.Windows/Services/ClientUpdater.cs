...
public class ClientUpdater : IClientUpdater
{
    ...
    private readonly ILogger<ClientUpdater> _logger;

    public ClientUpdater(ILogger<ClientUpdater> logger)
    {
        _logger = logger;
    }
    ...
    public void UpdateClient()
    {
        try
        {
            ...
            MapNetworkDrive(SharedFolder, Login, Password);
            DirectoryCopy($"{SharedFolder}/Client", $"{Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles)}/RemoteMaster/Client", true, true);
            CancelNetworkDrive(SharedFolder);
