...
public class FirewallService : IFirewallService
{
    public void AddRule(string name, NET_FW_ACTION action, NET_FW_IP_PROTOCOL protocol, NET_FW_PROFILE_TYPE2 profiles, NET_FW_RULE_DIRECTION direction, InterfaceType interfaceTypes, string? description = null, string? applicationPath = null, string? localAddress = null, string? localPort = null, string? remoteAddress = null, string? remotePort = null, string? service = null, bool edgeTraversal = false, string? icmpTypesAndCodes = null, object? interfaces = null, string? grouping = null)
    {
        ...
        newRule.Name = (BSTR)Marshal.StringToBSTR(name);
        newRule.Action = action;
        newRule.Enabled = action == NET_FW_ACTION.NET_FW_ACTION_ALLOW;
        newRule.Direction = direction;
        newRule.Protocol = (int)protocol;
        newRule.InterfaceTypes = (BSTR)Marshal.StringToBSTR(interfaceTypes.ToString());

        if (description != null)
        {
            newRule.Description = (BSTR)Marshal.StringToBSTR(description);
        }

        if (applicationPath != null)
        {
            newRule.ApplicationName = (BSTR)Marshal.StringToBSTR(applicationPath);
        }

        if (icmpTypesAndCodes != null)
        {
            newRule.IcmpTypesAndCodes = (BSTR)Marshal.StringToBSTR(icmpTypesAndCodes);
        }
        ...
        if (grouping != null)
        {
            newRule.Grouping = (BSTR)Marshal.StringToBSTR(grouping);
        }

        if (localAddress != null)
        {
            newRule.LocalAddresses = (BSTR)Marshal.StringToBSTR(localAddress);
        }

        if (localPort != null)
        {
            newRule.LocalPorts = (BSTR)Marshal.StringToBSTR(localPort);
        }

        if (remoteAddress != null)
        {
            newRule.RemoteAddresses = (BSTR)Marshal.StringToBSTR(remoteAddress);
        }

        if (remotePort != null)
        {
            newRule.RemotePorts = (BSTR)Marshal.StringToBSTR(remotePort);
        }

        if (service != null)
        {
            newRule.ServiceName = (BSTR)Marshal.StringToBSTR(service);
        }
    ...
    public void EnableRuleGroup(string groupName)
    {
        ...
        try
        {
            if (!fwPolicy2.IsRuleGroupEnabled(profileTypesBitmask, bstrGroupName))
            {
